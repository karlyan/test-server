...
# 单元测试（helm-unittest 插件，需 helm plugin install 一次；helm v4 必须加 --verify=false，因为
# helm-unittest 没有 signed release，否则报 "plugin source does not support verification"）
...
- `.github/workflows/lint-test.yaml` — PR + `main` push（仅当 `charts/**` / 此 workflow / `ct.yaml` 改动）触发 `ct lint` + `helm unittest`。注意 push 路径 `ct list-changed` 经常没列出 chart → ct lint 跳过；unittest 无条件跑。所有 chart 模板都必须有对应 suite。
